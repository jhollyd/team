...
const ProductManagement = () => {
  ...
  const handleImageUpload = async () => {
    ...
    if (!uploadFilename.trim()) {
      setError('Please enter a filename for the image');
      return;
    }
    ...
    try {
      const response = await fetch(
        `${import.meta.env.VITE_API_URL || 'http://localhost:3000'}/api/products/upload-image?filename=${encodeURIComponent(uploadFilename.trim())}`,
        {
          method: 'POST',
          headers: {
            'x-clerk-id': user?.id || ''
      ...
      setLocalImages(prev => [...prev, data.filename]);
      ...
      // Clear the filename input and file input
      setUploadFilename('');
                    ...
                    >
                      ...
                      {localImages.map((image) => (
                        <option key={image} value={image}>
                          {image.replace(/\.[^/.]+$/, '')} {/* Remove file extension */}
                        </option>
                      ))}
                    </select>
                  </div>
                    ...
                    <div className="space-y-2">
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          if (file) {
                            ...
                            // Auto-fill the filename field with the file name (without extension)
                            const fileNameWithoutExt = file.name.replace(/\.[^/.]+$/, "");
                            setUploadFilename(fileNameWithoutExt);
                          }
                        }}
                      ...
                      <div className="flex gap-2">
                        ...
                        <button
                          ...
                          disabled={!selectedFile || !uploadFilename.trim() || uploading}
